...
class Card:
    ...
    @staticmethod
    def get_barrel(l):  # Делаю метод статическим, работает со списком
        random.shuffle(l)  # Перемешиваю список
        for i in l[:]:  # Пробегаю по копии списка, чтобы не сбить индексацию
            yield l.pop(l.index(i))  # Удаляю и возвращаю из списка вызываемый элемент
